...
class GMVAE:
  ...
  def train_epoch(self, optimizer, data_loader):
    ...
    # average per batch
    total_loss /= num_batches*self.batch_size
    recon_loss /= num_batches*self.batch_size
    gauss_loss /= num_batches*self.batch_size
    cat_loss /= num_batches*self.batch_size
    ...
    # compute metrics
    accuracy = 100.0 * (self.metrics.cluster_acc(predicted_labels, true_labels)+self.alpha*(self.score-self.metrics.cluster_acc(predicted_labels, true_labels)))
